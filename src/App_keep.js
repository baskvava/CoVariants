...
const color_set = { "Alpha":"#8884d8", "Beta": "#82ca9d", "Gamma":"#FF8D33", "Delta":"#15CFBE", "Eta": "#F950CF",  "Kappa":"#187C33", "Iota":"#155972", "Lambda":"#232EE4",
  "Omicron":"#BFBF21", "others":"#B42DF8", "non_variants":"#EA2323"}

const rename_columns={"20I (Alpha, V1)": "Alpha", "20H (Beta, V2)":"Beta", "20J (Gamma, V3)":"Gamma",
  "21K (Omicron)":"Omicron", "21B (Kappa)":"Kappa","21D (Eta)":"Eta", "21G (Lambda)":"Lambda",
  "21F (Iota)":"Iota"}

const counties_list = ['California', 'Florida', 'Texas', 'Colorado', 'Minnesota', 'Massachusetts', 'New York', 'Washington', 'Michigan',
...
function getVarintsFromApiAsync() {
...
function One_Area(name){
  let fill = "url(#"+name+")"
  return(
        <Area type="monotone" dataKey={name} stroke={color_set[name]} fillOpacity={1} fill={fill} />
  )
}

function One_linearGradient(name){
  return(
      <>
        <linearGradient id={name} x1="0" y1="0" x2="0" y2="1">
          <stop offset="5%" stopColor={color_set[name]} stopOpacity={0.8}/>
          <stop offset="95%" stopColor={color_set[name]} stopOpacity={0}/>
        </linearGradient>
      </>
  )
}

function One_Plot(thisdata, ID){
  return(
      <>
              ...
              <ResponsiveContainer width='80%' height={500}>
                <AreaChart width={1200} height={450} data={thisdata}
                  ...
                  <defs>
                    ...
                    {/*</linearGradient>*/}
                    {(Object.keys(color_set)).map((virus) => (
                        One_linearGradient(virus)
                    ))}
                  </defs>
                  <XAxis dataKey="week" />
                  <YAxis />
...
function One_TabPane(county){
  return(
      <Tab.Pane eventKey={county}>
        { One_Plot(data, {county}) }
      </Tab.Pane>
  )
}

function Each_County( region ){
  return(
      <Tab.Container id={region} defaultActiveKey="first">
        <Row>
          <Col sm={2}>
            <Nav variant="pills" className="flex-column">
              <strong>{region}</strong>
              {(counties_regions_set[region]).map((county) => (
                  One_Nav(county)
              ))}
            </Nav>
          </Col>
...
function Each_Region(region, indx){
  ...
  return (
    <>
      <Navbar bg="primary" variant="dark">
        <Container>
          ...
          <Nav className="me-auto">
            ...
            <Nav.Link href="#regions">View by Counties</Nav.Link>
      ...
      <Container style={{"background":"whitesmoke"}}  fluid>
        ...
        { One_Plot(data, "ALL_USA") }
      </Container>

      <div id="regions">
        {
          Object.keys(counties_regions_set).map((region, idx) => (
          Each_Region(region,idx)
            ))}
      </div>
